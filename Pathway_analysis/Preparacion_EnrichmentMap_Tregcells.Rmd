...
Cargo inicialmente los listados de células Treg obtenidas en jVenn. 

```{r, cache = TRUE, warning=FALSE, message=FALSE}
setwd("~/Desktop/ELENA_UOC/TFM")
jVenn_Tregcells_vs_GSE22155_02 <- read_csv("DEGs/Venn_diagrams_cell_type_treatments_vs_untreated/jVenn_TregCells_vs_GSE22155_02.csv")
head(jVenn_Tregcells_vs_GSE22155_02) # sección de los resultados
jVenn_Tregcells_vs_GSE22155_47 <- read_csv("DEGs/Venn_diagrams_cell_type_treatments_vs_untreated/jVenn_TregCells_vs_GSE22155_47.csv")
jVenn_Tregcells_vs_GSE91061 <- read_csv("DEGs/Venn_diagrams_cell_type_treatments_vs_untreated/jVenn_TregCells_vs_GSE91061.csv")
jVenn_Tregcells_vs_GSE35640 <- read_csv("DEGs/Venn_diagrams_cell_type_treatments_vs_untreated/jVenn_TregCells_vs_GSE35640.csv")
jVenn_Tregcells_vs_GSE61992 <- read_csv("DEGs/Venn_diagrams_cell_type_treatments_vs_untreated/jVenn_TregCells_vs_GSE61992.csv")
setwd("~/Desktop/ELENA_UOC/TFM/GOBPs") # Directorio de trabajo
jVenn_Tregcells <- read_delim("~/Desktop/ELENA_UOC/TFM/DEGs/Venn_diagrams_cell_type_treatments_comparison//Valid/jVenn_chart_Tregcells.csv", 
...
```{r, cache = TRUE, warning=FALSE, message=FALSE}
multi_gostres <- gost(query = list("GSE22155_02" = unique(jVenn_Tregcells_vs_GSE22155_02$GSE50509[which(is.na(jVenn_Tregcells_vs_GSE22155_02$GSE50509) == F)]),
                             "GSE22155_47" = jVenn_Tregcells_vs_GSE22155_47$GSE50509[which(is.na(jVenn_Tregcells_vs_GSE22155_47$GSE50509) == F)],
                             "GSE35640" = jVenn_Tregcells_vs_GSE35640$GSE50509[which(is.na(jVenn_Tregcells_vs_GSE35640$GSE50509) == F)],
                             "GSE91061" = jVenn_Tregcells_vs_GSE91061$GSE50509[which(is.na(jVenn_Tregcells_vs_GSE91061$GSE50509) == F)],
                             "GSE61992" = jVenn_Tregcells_vs_GSE61992$GSE50509[which(is.na(jVenn_Tregcells_vs_GSE61992$GSE50509) == F)]
                             ),
...
gem %>% group_by(query) %>%
  group_walk(~
    write.table(data.frame(.x[,c("GO.ID", "Description", "p.Val", "FDR", "Phenotype", "Genes")]), 
                file = paste0("./Venn_diagrams_cell_type_treatments_vs_untreated/gProfiler_Tregcells_Only_geneset_", unique(.y$query), "_gem.txt"),
...
write.table(df, file = "./Venn_diagrams_cell_type_treatments_vs_untreated/Tregcells_GOBPs.txt", sep = "\t",quote = F,row.names = F)
write.table(df2, file = "./Venn_diagrams_cell_type_treatments_vs_untreated/Tregcells_GOBPs_freq.txt", sep = "\t",quote = F,row.names = F)
#rm(jVenn_Tregcells_vs_GSE22155_02, jVenn_Tregcells_vs_GSE22155_47, jVenn_Tregcells_vs_GSE35640, jVenn_Tregcells_vs_GSE61992, jVenn_Tregcells_vs_GSE91061)
```
...
```{r, cache = TRUE, warning=FALSE, message=FALSE}
multi_gostres <- gost(query = list("GSE61992" = jVenn_Tregcells$GSE61992[which(is.na(jVenn_Tregcells$GSE61992) == F)],
                             "GSE91061" = jVenn_Tregcells$GSE91061[which(is.na(jVenn_Tregcells$GSE91061) == F)]
                             ),
                      evcodes = TRUE, multi_query = FALSE, 
                      sources = c("GO", "REAC", "MIRNA", "CORUM", "HP", "HPA", "WP"))
head(multi_gostres$result[,1:7])
...
gem %>% group_by(query) %>%
  group_walk(~
    write.table(data.frame(.x[,c("GO.ID", "Description", "p.Val", "FDR", "Phenotype", "Genes")]), 
                file = paste0("./Treatment_comparisons/gProfiler_Tregcells_Only_geneset_", unique(.y$query), "_gem.txt"),
                sep = "\t", quote = F, row.names = F))
```
...
df <- as.data.frame(multi_gostres$result[,1:13])
table(df$query)
write.table(gem, file = "./Treatment_comparisons/gProfiler_Tregcells_Only_genesets.txt", sep = "\t",quote = F,row.names = F)
```
...
Primero realizo el pathway multianálisis, pero no se obtiene resultado: "Empty query. Please double check your input." Sólo hay 5 genes, lo que debe impedir obtener un resultado.

```{r, cache = TRUE, warning=FALSE, message=FALSE}
#multi_gostres <- gost(query = list("TabrafenibTrametinib_antiCTLA4PD1" = jVenn_Tregcells$`GSE91061|GSE61992`[which(is.na(jVenn_Tregcells$`GSE61992|GSE91061`) == F)]
#),
#                      evcodes = TRUE, multi_query = FALSE, 
#                      sources = c("GO", "REAC", "MIRNA", "CORUM", "HP", "HPA", "WP"))
#head(multi_gostres$result[,1:7])
