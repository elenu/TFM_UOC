...
Cargo inicialmente los listados de células NK obtenidas en jVenn. 

```{r, cache = TRUE, warning=FALSE, message=FALSE}
setwd("~/Desktop/ELENA_UOC/TFM")
jVenn_NKcells_vs_GSE22155_02 <- read_csv("DEGs/Venn_diagrams_cell_type_treatments_vs_untreated/jVenn_NKCells_vs_GSE22155_02.csv")
head(jVenn_NKcells_vs_GSE22155_02) # sección de los resultados
jVenn_NKcells_vs_GSE22155_47 <- read_csv("DEGs/Venn_diagrams_cell_type_treatments_vs_untreated/jVenn_NKCells_vs_GSE22155_47.csv")
jVenn_NKcells_vs_GSE91061 <- read_csv("DEGs/Venn_diagrams_cell_type_treatments_vs_untreated/jVenn_NKCells_vs_GSE91061.csv")
jVenn_NKcells_vs_GSE35640 <- read_csv("DEGs/Venn_diagrams_cell_type_treatments_vs_untreated/jVenn_NKCells_vs_GSE35640.csv")
jVenn_NKcells_vs_GSE61992 <- read_csv("DEGs/Venn_diagrams_cell_type_treatments_vs_untreated/jVenn_NKCells_vs_GSE61992.csv")
setwd("~/Desktop/ELENA_UOC/TFM/GOBPs") # Directorio de trabajo
jVenn_NKcells <- read_delim("~/Desktop/ELENA_UOC/TFM/DEGs/Venn_diagrams_cell_type_treatments_comparison/Valid/jVenn_chart_NKCells.csv", 
...
```{r, cache = TRUE, warning=FALSE, message=FALSE}
multi_gostres <- gost(query = list("GSE22155_02" = unique(jVenn_NKcells_vs_GSE22155_02$GSE50509[which(is.na(jVenn_NKcells_vs_GSE22155_02$GSE50509) == F)]),
                             "GSE22155_47" = jVenn_NKcells_vs_GSE22155_47$GSE50509[which(is.na(jVenn_NKcells_vs_GSE22155_47$GSE50509) == F)],
                             "GSE35640" = jVenn_NKcells_vs_GSE35640$GSE50509[which(is.na(jVenn_NKcells_vs_GSE35640$GSE50509) == F)],
                             "GSE91061" = jVenn_NKcells_vs_GSE91061$GSE50509[which(is.na(jVenn_NKcells_vs_GSE91061$GSE50509) == F)],
                             "GSE61992" = jVenn_NKcells_vs_GSE61992$GSE50509[which(is.na(jVenn_NKcells_vs_GSE61992$GSE50509) == F)]
                             ),
...
gem %>% group_by(query) %>%
  group_walk(~
    write.table(data.frame(.x[,c("GO.ID", "Description", "p.Val", "FDR", "Phenotype", "Genes")]), 
                file = paste0("./Venn_diagrams_cell_type_treatments_vs_untreated/gProfiler_NKcells_Only_geneset_", unique(.y$query), "_gem.txt"),
...
write.table(df, file = "./Venn_diagrams_cell_type_treatments_vs_untreated/NKcells_GOBPs.txt", sep = "\t",quote = F,row.names = F)
write.table(df2, file = "./Venn_diagrams_cell_type_treatments_vs_untreated/NKcells_GOBPs_freq.txt", sep = "\t",quote = F,row.names = F)
#rm(jVenn_NKcells_vs_GSE22155_02, jVenn_NKcells_vs_GSE22155_47, jVenn_NKcells_vs_GSE35640, jVenn_NKcells_vs_GSE61992, jVenn_NKcells_vs_GSE91061)
```
...
```{r, cache = TRUE, warning=FALSE, message=FALSE}
multi_gostres <- gost(query = list("GSE61992" = jVenn_NKcells$GSE61992[which(is.na(jVenn_NKcells$GSE61992) == F)],
                             "GSE91061" = jVenn_NKcells$GSE91061[which(is.na(jVenn_NKcells$GSE91061) == F)]
                             ),
                      evcodes = TRUE, multi_query = FALSE, 
                      sources = c("GO", "REAC", "MIRNA", "CORUM", "HP", "HPA", "WP"))
head(multi_gostres$result[,1:7])
...
gem %>% group_by(query) %>%
  group_walk(~
    write.table(data.frame(.x[,c("GO.ID", "Description", "p.Val", "FDR", "Phenotype", "Genes")]), 
                file = paste0("./Treatment_comparisons/gProfiler_NKcells_Only_geneset_", unique(.y$query), "_gem.txt"),
                sep = "\t", quote = F, row.names = F))
```
...
df <- as.data.frame(multi_gostres$result[,1:13])
table(df$query)
write.table(gem, file = "./Treatment_comparisons/gProfiler_NKcells_Only_genesets.txt", sep = "\t",quote = F,row.names = F)
```
...
multi_gostres <- gost(query = list(
                                   "antiCTLA4PD1_TabrafenibTrametinib" = jVenn_NKcells$`GSE91061|GSE61992`[which(is.na(jVenn_NKcells$`GSE91061|GSE61992`) == F)]
                                   ),
                      evcodes = TRUE, multi_query = FALSE, 
                      sources = c("GO", "REAC", "MIRNA", "CORUM", "HP", "HPA", "WP"))
head(multi_gostres$result[,1:7])
...
gem %>% group_by(query) %>%
  group_walk(~
                   write.table(data.frame(.x[,c("GO.ID", "Description", "p.Val", "FDR", "Phenotype", "Genes")]), 
                file = paste0("./Treatment_comparisons/gProfiler_NKcells_maxOverlap_geneset_", unique(.y$query), "_gem.txt"),
                sep = "\t", quote = F, row.names = F))
```
...
En este caso no hay resultado de GOBPs.
