...
En este análisis utilizo los datos del estudio GSE120575 descargados mediante la función ```getGEO``` des de la base de datos GEO, del NCBI: <https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE120575>. 

Las muestras consisten en 16291 muestras analizadas con la plataformas GPL18573.
...
gex <- gex[,-1]
gex <- apply(gex, 2, as.numeric)
rownames(gex) <- GSE120575_Sade_Feldman_melanoma_single_cells_TPM_GEO$...1[-1]

keep <- rownames(gex)[(rowSums(gex) > 0)]
gex <- gex[rownames(gex) %in% keep,]
...
GSE120575_metadata <- read_excel("GSE120575_Tabl1.xlsx", 
    sheet = "Patient-scRNA data", skip = 1)

Final_metadata_GSE120575 <- read_excel("Final_metadata_GSE120575.xlsx")

GSE120575_Tabl1 <- read_excel("GSE120575_Tabl1.xlsx", 
    sheet = "Unsupervised clusters (%)-Fig 1")

cell_types <- as.data.frame(cbind("Cluster"= c("G1","G2","G3", "G4","G5", "G6", "G7", "G8", "G9", "G10", "G11"), "Cell_type"= c("B cells","Plasma cells","Monocytes/Macrophages","Dendritic cells","Lymphocytes","Exhausted CD8 T cells", "Regulatory T cells", "Cytotoxicity (Lymphocytes)", "Exhausted/HS CD8 T cells", "Memory T cells", "Lymphocytes exhausted/cell-cycle")))


rownames(GSE120575) <- GSE120575$Sample
setwd("~/Desktop/ELENA_UOC/TFM")
design <- apply(GSE120575[,2:12],2,as.numeric) # Primero, las fracciones celulares

# Seguido de confounders como los eventos de supervivencia:
GSE120575$OS_events <- 1# Harmonización de eventos de supervivencia
GSE120575$OS_events[grep("Alive",GSE120575$`patient last status:ch1`)] <- 0# Harmonización de eventos de supervivencia
GSE120575$OS_events <- (GSE120575$`survival from primary melanoma (months):ch1`)*30# Harmonización de días de supervivencia

design <- as.data.frame(cbind(OS=GSE120575$OS_events,design)) # Unión de la tabla con los eventos definidos
design$Samples <- GSE120575$Sample

# Género de los pacientes (1= hombre, 0= mujer)
design$sex_male <- ifelse(GSE120575$`patient sex:ch1` == "Male", 1, 0)
...
design$stage_3 <- ifelse(GSE120575$`stage at primary diagnosis 5th edition:ch1` == "Stage III", 1, 0)
...
saveWorkbook(wb, "Linear_model_results_GSE120575_stats.xlsx", overwrite = TRUE)
saveWorkbook(gopbs, "Linear_model_results_GSE120575_GOBPs.xlsx", overwrite = TRUE)
```
