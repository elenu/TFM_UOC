...
```{r, cache = TRUE, warning=FALSE, message=FALSE}
sc_matrix <- as.matrix(sc.eset@assayData$exprs)
bk_matrix <- as.matrix(bulk.eset@assayData$exprs)
bk_matrix2 <- bk_matrix[rownames(bk_matrix) %in% rownames(sc_matrix),]

solDWLS<-solveDampenedWLS(sc_matrix,bk_matrix2) #Error in t(S) %*% B : non-conformable arguments
