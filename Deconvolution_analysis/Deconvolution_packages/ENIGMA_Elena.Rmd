---
title: |
  | Deconvolución de datos de mealoma con ENIGMA:
...
Primero creamos el objeto de enigma para trabajar.

```{r, cache = TRUE, warning=FALSE, message=FALSE}
bulk.mtx2 <- as.matrix(bulk.mtx[rownames(bulk.mtx) %in% rownames(sc_gex3),])
probes_bulk <- rownames(bulk.mtx2)
sc_gex4 <- rbind(t(sc_metadata),sc_gex3)
egm = create_ENIGMA(bulk = bulk.mtx2, ref = as.matrix(sc_gex3),ref_type = "single_cell", meta_ref = sc_metadata)
egm
```

Una vez se ha preparado el objeto ENIGMA, se puede calcular las proporciones celulares mediante em método de regresión lineal robusta, y normalizar mediante el cálculo de logaritmos. 

```{r, cache = TRUE, echo=FALSE, warning=FALSE, message=FALSE}
############egm = get_cell_proportion(egm, method = "RLR") ### Terminated after > 1h!!!!
