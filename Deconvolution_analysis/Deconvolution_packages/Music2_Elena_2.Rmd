...
#}
#devtools::install_github("meichendong/dtangle")
...
Primero creamos el los objetos de MUSIC para trabajar, los cuales necesito que tengan los mismos genes (el mismo fenotipo).

```{r, cache = TRUE, warning=FALSE, message=FALSE}
bulk_metadata <- as.data.frame(gset@phenoData@data) # Paso la metadata disponible a una tabla
bulk.pheno <- data.frame(check.names=F, check.rows=F, stringsAsFactors=F, row.names=bulk_metadata$geo_accession, 
                       SubjectName=bulk_metadata$geo_accession, Response = bulk_metadata$`treatment response for gene profiling:ch1`) 
bulk.meta <- data.frame(labelDescription=c("SampleID", "Response"),row.names=c("geo_accession","treatment response for gene profiling:ch1")) # Metadata para identificar las columnas
bulk.pdata <- new("AnnotatedDataFrame",data=bulk.pheno,varMetadata=bulk.meta) # Este parámetro contiene ambos e irá en el objeto ExpressionSet
...
bulk.mtx2 <- bulk.mtx[rownames(bulk.mtx) %in% rownames(sc_gex3),]
```


```{r, cache = TRUE, warning=FALSE, message=FALSE}
# Convertir los datos de expresión del bulk RNA-seq a objeto ExpressionSet:
bulk.eset <- Biobase::ExpressionSet(assayData = as.matrix(as.data.frame(bulk.mtx2)), phenoData=bulk.pdata)
#print("ExpressionSet object:")
bulk.eset
#eset <- getESET(counts, fdata = fdata, pdata = pdata)
#class(eset)
...
est = music2_prop_t_statistics(bulk.control.mtx = as.matrix(bulk.eset@assayData$exprs[,colnames(bulk.eset@assayData$exprs) %in% bulk_metadata$geo_accession[bulk_metadata$`treatment response for gene profiling:ch1` == "R"]]), 
                               bulk.case.mtx = as.matrix(bulk.eset@assayData$exprs[,colnames(bulk.eset@assayData$exprs) %in% bulk_metadata$geo_accession[bulk_metadata$`treatment response for gene profiling:ch1` == "NR"]]), 
                               sc.sce = sc.eset, clusters = 'Cell_type', samples = 'SampleID', select.ct = c(unique(sc_metadata$Cell_type)), n_resample=20, sample_prop=0.5,cutoff_c=0.05,cutoff_r=0.01) # ERROR: Error in bulk.control.mtx[gene.bulk, ] : subscript out of bounds

#est.prop = est$Est.prop

# music2 deconvolution
est.prop = music2_prop(bulk.eset = bulk.eset, sc.eset = sc.eset, condition='Response', control='R',case='NR', clusters = 'Cell_type', samples = 'SubjectName', select.ct = c(unique(sc_metadata$Cell_type)), n_resample=20, sample_prop=0.5,cutoff_c=0.05,cutoff_r=0.01)$Est.prop # ERROR:  argument "bulk.mtx" is missing, with no default

# MuSiC
bulk.mtx3 = exprs(bulk.eset)
call(rownames(bulk.mtx3), rownames(sc.eset@assayData$exprs))

prop_music=music_prop(bulk.mtx = bulk.mtx3, sc.sce = sc.eset, 
                      clusters = 'Cell_type', samples = 'SubjectName',
                      select.ct = as.vector(c(unique(sc_metadata$Cell_type))), verbose = F)$Est.prop.weighted
